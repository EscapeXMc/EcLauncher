...
import { Search, Download, ExternalLink, ChevronDown, ChevronUp, AlertCircle, Layers, Eye } from "lucide-react";
...
export function PacksPage() {
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    setHasSearched(true);
    try {
      const hits = await api.mods.search(q, "resourcepack", undefined, undefined, 20);
      setResults(hits);
    } catch (err: any) {
  ...
  const handleInstall = async (slug: string) => {
    ...
    setInstalling(slug);
    try {
      await api.mods.installResourcepack(targetInstance.name, slug);
    } catch (err) {
  ...
  const handleInstallVersion = async (slug: string, versionId: string) => {
    ...
    try {
      await api.mods.installResourcepack(targetInstance.name, slug, versionId);
    } catch (err) {
      console.error("Install failed:", err);
    }
    setInstalling(null);
  ...
  return (
    <div className="space-y-5 max-w-4xl mx-auto">
      <div className="flex items-center gap-2">
        <Layers size={20} style={{ color: themeColors.accent }} />
        <div>
          <h1 className="tinycaps text-xl font-bold gradient-text" style={{ "--accent": themeColors.accent, "--accent2": themeColors.purple || themeColors.blue } as any}>Resource Packs</h1>
          <p className="text-xs mt-0.5" style={{ color: themeColors.text_muted }}>Browse and install resource packs for your profiles</p>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          <GlassInput value={query} onChange={setQuery} onKeyDown={handleKeyDown}
            placeholder="Search resource packs (e.g. better textures, faithful)..." />
        </div>
      ...
      <div className="space-y-2">
        {results.map((pack) => {
          ...
          return (
            <GlassCard key={pack.slug} hover>
              <div className="flex items-start gap-3 cursor-pointer" onClick={() => setExpanded(isOpen ? null : pack.slug)}>
                {pack.icon_url && <img src={pack.icon_url} alt={pack.title} className="w-10 h-10 rounded-lg object-cover shrink-0" />}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="tinycaps text-sm font-semibold truncate" style={{ color: themeColors.text_main }}>{pack.title}</h3>
                    {isOpen ? <ChevronUp size={14} style={{ color: themeColors.text_muted }} /> : <ChevronDown size={14} style={{ color: themeColors.text_muted }} />}
                  </div>
                  <p className="text-xs mt-0.5 line-clamp-2" style={{ color: themeColors.text_sub }}>{pack.description}</p>
                  <div className="flex items-center gap-3 mt-1.5 text-[10px]" style={{ color: themeColors.text_muted }}>
                    <span style={{ color: themeColors.accent }}>↓ {pack.downloads?.toLocaleString() || 0}</span>
                    {pack.categories && <span>{pack.categories.slice(0, 3).join(", ")}</span>}
                  </div>
                </div>
              </div>
              {isOpen && (
                <div className="mt-3 pt-3 flex gap-2 items-center" style={{ borderTop: `1px solid ${themeColors.border}` }}>
                  <GlowButton size="sm" onClick={(e) => { e.stopPropagation(); handleView(pack.slug); }} className="flex items-center gap-1">
                    <Eye size={12} /> View
                  </GlowButton>
                  <GlowButton size="sm" onClick={(e) => { e.stopPropagation(); handleInstall(pack.slug); }}
                    disabled={!targetInstance || installing === pack.slug} className="flex items-center gap-1">
                    <Download size={12} /> {installing === pack.slug ? "Installing..." : "Install"}
                  </GlowButton>
                </div>
              )}
            </GlassCard>
        ...
        {!searching && results.length === 0 && hasSearched && (
          <div className="text-center py-12">
            <Layers size={32} className="mx-auto mb-2" style={{ color: themeColors.text_muted, opacity: 0.4 }} />
            <p className="text-sm" style={{ color: themeColors.text_muted }}>No resource packs found for "{query}"</p>
          </div>
        )}
        ...
        {!searching && results.length === 0 && !hasSearched && (
          <div className="flex items-center justify-center py-12">
            <div className="w-6 h-6 border-2 rounded-full animate-spin" style={{ borderColor: themeColors.accent, borderTopColor: "transparent" }} />
            <span className="ml-3 text-sm" style={{ color: themeColors.text_muted }}>Loading popular packs...</span>
          </div>
        )}
      </div>
    </div>
